...
var resizePizzas = function(size) { 
  ...
  // Remove All Unneeded Functions. A Simple Set to the Width Style is Only Needed.
  function changePizzaSizes (size) {
    ...
    // Can't Find the Variables, Check Out DOMContentLoaded Evt Listener
...
function updatePositions() {
  ...
  // Create Array for Phases
  var phase = [];

  // Removed Calculation Outside of For Loop
  var scrollInt = document.body.scrollTop / 1250;

  // Build Array of Phases
  for (var k = 0; k < 5; k++) {
    phase.push(Math.sin((scrollInt) + k) * 100);
  }

  // Cycle through 5 Values in Phase Array and Apply to Left Style
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Calculation to Produce Pizzas in the BG based on Screen's Height
  var numberOfPizzas = (screen.availHeight / s) * cols;

  for (var i = 0; i < numberOfPizzas; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza-sm.png"; // Optimized and Resized Image
    elem.basicLeft = (i % cols) * s;
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    document.getElementById("movingPizzas1").appendChild(elem);
  }

  // Removed out of For loop and Generated on DOM Load
  randomPizzas = document.getElementsByClassName("randomPizzaContainer");
  randomPizzasLen = randomPizzas.length;
...
Added the following to the mover Class in style.css.
Allows each pizza image to work on its own layer.
