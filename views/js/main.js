...
// Simplified resizePizzas function by taking advantage of switch to find new pizza size.
var resizePizzas = function(size) {
	...
	switch(size) {
		case "1":
			...
			newsize = Math.round(0.25 * windowwidth);
			break;
		case "2":
			document.querySelector("#pizzaSize").innerHTML = "Medium";
			newsize = Math.round(0.3333 * windowwidth);
			break;
		case "3":
			document.querySelector("#pizzaSize").innerHTML = "Large";
			newsize = Math.round(0.5 * windowwidth);
			break;
		default:
	...
	// getElementsByClassName() supposed to be faster than querySelectorAll()
	var randPizCont = document.getElementsByClassName("randomPizzaContainer");
	var randPizContLen = randPizCont.length;
	for (var i = 0; i < randPizContLen; i++) {
		randPizCont[i].style.width = newsize + 'px';
	}
...
function updatePositions() {
	...
	/*
		bodyScroll is set outside of the loop, since its value is
		pretty much the same for every element in <body> at a given time;
		a phase can have only one of five consecutive distinct numbers, so
		an array is created to pick those numbers from.
	*/
	var bodyScroll = document.body.scrollTop / 1250,
			phases = [
		Math.sin(bodyScroll + 0) * 100,
		Math.sin(bodyScroll + 1) * 100,
		Math.sin(bodyScroll + 2) * 100,
		Math.sin(bodyScroll + 3) * 100,
		Math.sin(bodyScroll + 4) * 100
	];

	// set new position for each of items, looping
	// through phases array
	for (var j = 0, k = 0; j < itemsLen; j++) {
		items[j].style.left = items[j].basicLeft + phases[k] + 'px';
		k = (k < 4 ? k + 1 : 0);
	}
...
var items = [],			// Holds sliding pizzas DOM elements
		itemsLen = 20;	// Number of sliding pizzas, don't need too many
										// 20 should work for decent viewports
document.addEventListener('DOMContentLoaded', function() {
	...
	for (var i = 0; i < itemsLen; i++) {
		...
		// Populate array with moving pizzas, do it once
		// instead of each time updatePositions function runs
		items.push(elem);
	}
	updatePositions();
});
