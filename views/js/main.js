...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
          ...
          // Selects the first .randomPizzaContainer and passes it to determinDx function
            var randomPizza = document.querySelectorAll('.randomPizzaContainer');
            var dx = determineDx(randomPizza[0], size);

          //Selects the first width of pizza element
            var pizzawidth = (randomPizza[0].offsetWidth + dx) + 'px';

          // The number of .randomPizzaContainer elements that we will be using as the number of iterations in our for loop
            var randomPizzaLength = randomPizza.length;

          // Loops through all the .randomPizzaContainer elements
            for (var i = 0; i < randomPizzaLength; i++) {
              randomPizza[i].style.width = pizzawidth;
            }
          }
...
function updatePositions() {
  ...
  // This calculation only needs to happen once so I took it out of the for loop. If not, the calculation per iteration slows down the speed of the page.
...
// Updated variable to 60. Enough pizzas to cover a 1920x1080 screen.
  for (var i = 0; i < 60; i++) {
