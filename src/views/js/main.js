...
var resizePizzas = function(size) {
  ...
  // OPTIMIZATION: windowwidth variable moved outside of determineDx to limit DOM access
  ...
  function changePizzaSizes(size) {
    // OPTIMIZATION: dx and newwidth taken outside of for loop,
    // determineDx only needs to be run once since all pizzas are the same size
...
// OPTIMIZATION: pizzasDiv variable removed from for loop, only needs to be created once
...
function updatePositions() {
  ...
  // OPTIMIZATION: Precalculates the 5 different possible phase values
  var items = document.getElementsByClassName('mover');
  var top = document.body.scrollTop / 1250;
  var phase1 = Math.sin(top);
  var phase2 = Math.sin(top + 1);
  var phase3 = Math.sin(top + 2);
  var phase4 = Math.sin(top + 3);
  var phase5 = Math.sin(top + 4);
  // OPTIMIZATION: Caches items.length
  for (var i = 0, len = items.length; i < len; i++) {
    switch(i % 5) {
      ...
      case 3:
        ...
        break;
      case 4:
        items[i].style.left = items[i].basicLeft + 100 * phase5 + 'px';
        break;
      default:
...
// Generates sliding pizzas
function generatePizzas() {
  var page = document.getElementById('movingPizzas1');
  page.innerHTML = '';
  var h = window.innerHeight;
  var cols = 8;
  var s = 256;

  // OPTIMIZATION: Stops generating pizzas when topValue would place them outside
...
// Run generatePizzas when the page loads or resizes
document.addEventListener('DOMContentLoaded', generatePizzas);
window.addEventListener('resize', generatePizzas);